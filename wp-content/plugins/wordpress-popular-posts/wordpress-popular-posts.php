...
if ( !class_exists('WordpressPopularPosts') ) {
	...
	class WordpressPopularPosts extends WP_Widget {
		...
		function WordpressPopularPosts() {
			...
			// widget settings
			$widget_ops = array( 'classname' => 'popular-posts widget_popular_posts', 'description' => __('The most Popular Posts on your blog.', 'wordpress-popular-posts') );
				...
				foreach($mostpopular as $p) {
					...
					if ( $instance['post-excerpt']['active'] ) {
						...
						if ( !empty($excerpt) ) {
							...
							if ( !$instance['markup']['custom_html'] ) {
								$excerpt = "<span class=\"wpp-excerpt\" style='display:inline'>" . $excerpt . "</span>";
							}
					...
					if ($instance['markup']['custom_html']) { // build custom layout
						...
						$content .= "<div class='item-post'><div class='post-number'>$count</div><div class='post-right'>{$thumb}<h4><a href=\"{$permalink}\" title=\"{$title}\" class=\"wpp-post-title\"><strong>{$title_sub}</strong><br/>{$excerpt}</a><span class=\"post-stats\">{$stats}</span>{$rating}</h4></div></div>" . "\n";
					}
          $count++;
				}
